                        ...
                        <div class="row">
                            <div class="col-md-6 mb-2">
                                <div class="form-group">
                                    <input type="text" name="name" id="service_type" class="form-control form-control-md shadow-none outline-none" placeholder="Phone" required >
                                </div>
                            </div>
                            <div class="col-md-6 mb-2">
                                ...
                                <td>
                                    <div class="d-flex justify-content-center gap-2">
                                        <a href="javascript:void(0)" class="nav-link btn btn-sm rounded-0 edit_form_btn" 
                                            ...
                                            data-status = "{{$phone->status_id}}"
                                            data-bs-target="#editbox"><i class="fas fa-tools "></i>
                                        </a>

                                        <a href="javascript:void(0)" class="btn btn-sm secondary rounded-0 delete-btns" 
                                            data-id="{{$phone->id}}" 
                                            data-name="{{$phone->name}}" ><i class="fas fa-trash "
                                            ></i>
                                        </a>
                                    </div>

                                </td>
                                <form id="formdelete{{$phone->id}}" action="{{route('phones.destroy',$phone->id)}}" method="POST">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
                                </form>
                            </tr>
    ...
    <script>
        $(document).ready(function(){
            ...
            $(".edit_form_btn").click(function(){
                ...
                $("#edit_form_container").attr("action",`/phones/${getId}`);
            })
        })
    </script>
@endsection
